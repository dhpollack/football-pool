...
test.describe('Logout UI', () => {
  test('should show user menu and allow logout with admin user', async ({ page }) => {
    // Login with admin user (who has a player profile)
    await page.goto('/login');
    await page.fill(E2E_CONFIG.SELECTORS.LOGIN.EMAIL, E2E_CONFIG.ADMIN_CREDENTIALS.email);
    await page.fill(E2E_CONFIG.SELECTORS.LOGIN.PASSWORD, E2E_CONFIG.ADMIN_CREDENTIALS.password);
    ...
    await expect(page.locator(E2E_CONFIG.SELECTORS.USER_MENU.PROFILE_LINK)).toBeVisible();
    await expect(page.locator(E2E_CONFIG.SELECTORS.USER_MENU.LOGOUT_BUTTON)).toBeVisible();
