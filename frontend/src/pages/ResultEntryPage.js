...
const ResultEntryPage = () => {
  ...
  return (
    <div>
      ...
      {games.map((game) => (
        <div key={game.ID}>
          <h3>{game.FavoriteTeam} vs {game.UnderdogTeam}</h3>
          <input
            type="number"
            placeholder={`${game.FavoriteTeam} Score`}
            onChange={(e) => handleScoreChange(game.ID, 'favorite_score', parseInt(e.target.value))}
          />
          <input
            type="number"
            placeholder={`${game.UnderdogTeam} Score`}
            onChange={(e) => handleScoreChange(game.ID, 'underdog_score', parseInt(e.target.value))}
          />
