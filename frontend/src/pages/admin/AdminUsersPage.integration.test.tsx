...
describe("AdminUsersPage (Integration)", () => {
  ...
  it("should render the page and display a list of users", async () => {
    server.use(getAdminListUsersMockHandler200({
      users: [{
        id: 1,
  ...
  it("should paginate through the list of users", async () => {
    ...
    server.use(
      getAdminListUsersMockHandler200((info) => {
        ...
        return { 
          users, 
          pagination: { total: totalUsers, page: currentPage, limit: currentLimit } 
        };
      })
    );
  ...
  it("should edit an existing user", async () => {
    ...
    server.use(
      getAdminListUsersMockHandler200({ users: [initialUser], pagination: { total: 1, page: 1, limit: 20 } }),
  ...
  it("should delete a user", async () => {
    const initialUser = { id: 1, email: "userToDelete@example.com", name: "User To Delete", role: "user", pick_count: 0, total_wins: 0, created_at: "2024-01-01T00:00:00Z", updated_at: "2024-01-01T00:00:00Z" };
    server.use(
      getAdminListUsersMockHandler200({ users: [initialUser], pagination: { total: 1, page: 1, limit: 20 } }),
    ...
    expect(await screen.findByText("userToDelete@example.com")).toBeInTheDocument();
    ...
    await waitFor(() => {
      expect(screen.getByText("userToDelete@example.com")).toBeInTheDocument();
    }, { timeout: 10000 });
  });

  it("should show empty state when API returns no users", async () => {
    server.use(getAdminListUsersMockHandler200({ users: [], pagination: { total: 0, page: 1, limit: 20 } }));
  ...
  it("should handle API errors gracefully", async () => {
    server.use(getAdminListUsersMockHandler401({ error: "Unauthorized", message: "Failed to load users" }));
